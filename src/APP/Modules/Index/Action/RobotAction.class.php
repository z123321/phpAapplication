<?php  
	...
	Class RobotAction extends CommonAction{
        ...
        public function buy(){
            ...
            if(empty($data)){
                alert('Solar PV does not exist',U('Robot/index'));
            }
            ...
            if($my_gounum >=$data['xiangou']){
                echo '<script>alert("Has reached the current upper limit of solar photovoltaic！");window.history.back(-1);</script>';
                die;
            }
            if($data['stock'] < 1){
                echo '<script>alert("The solar photovoltaic has been purchased, please come back another day！");window.history.back(-1);</script>';
                die;
            }
            $faquan = C('faquan');
            ...
            if($id == $faquan) {
                ...
                if ($jifen < $data['price']) {
                    alert('Insufficient gold coins in the account, go and earn gold coins!', U('task/index'));
                }
                ...
                if ($jinbi < $data['price']) {
                    alert('Insufficient account balance, please recharge first', U('wallet/onlinerecharge'));
                }

                M("member")->where(array('username' => session('username')))->setDec('money', $data['price']);
                account_log(session('username'), $data['price'], 'buy' . $data['title'], 0);
                $map = array();
                $map['kjbh'] = $letter . date('d') . substr(time(), -5) . sprintf('%02d', rand(0, 99));
                $map['user'] = $username;
                $map['tuijian'] = $tuijian;
                ...
                if (M('order')->add($map)) {
                    ...
                    if ($parentcount > 0) {
                        ...
                        account_log($parent, $one, '1 level purchase reward', 1, 2, 1, 0, $username);

                    } else {
                        M("member")->where(array('username' => $parent))->setInc('money', $ones);
                        account_log($parent, $ones, '1 level purchase reward', 1, 2, 1, 0, $username);
                    }

                    if ($parentcount1> 0) {
                        M("member")->where(array('username' => $parent1))->setInc('money', $two);
                        account_log($parent1, $two,'Level 2 purchase reward', 1, 2, 1, 0, $username);

                    } else {
                        M("member")->where(array('username' => $parent1))->setInc('money', $twos);
                        account_log($parent1, $twos,'Level 2 purchase reward', 1, 2, 1, 0, $username);
                    }
                    if ($parentcount2> 0) {
                        M("member")->where(array('username' => $parent2))->setInc('money', $three);
                        account_log($parent2, $three,'Level 3 purchase reward', 1, 2, 1, 0, $username);

                    } else {
                        M("member")->where(array('username' => $parent2))->setInc('money', $threes);
                        account_log($parent2, $threes,'Level 3 purchase reward', 1, 2, 1, 0, $username);
                    }
                }
                $user_id = session('mid');
            ...
            alert('uccessful purchase of solar photovoltaic', U('Robot/robot'));
        }
        ...
        public function jiesuan(){
            ...
            $username=session('username');
            if(empty($id)){
                alert('lost！',U('Robot/robot'));
            }

            $order=M('order')->where("id = {$id} and zt = 1 and user_id = {$user_id}")->find();

            if(empty($order)){
                $this->ajaxReturn(array('result'=>0,'info'=>'The solar photovoltaic does not exist！'));
            }
            //判断与上次结算时间有没有达到24小时
            $jiesuan_time=C('jiesuan_time');
            ...
            if(time()-$order['UG_getTime'] < $jiesuan_time*3600){
                alert('Time left:'.$jiesuan_time.'hour！',U('Robot/robot'));
            }
            //算出已经结算的时间
            ...
            account_log($username,$shouyi,'Income',1,1,1,$order['id']);
            ...
            account_log($parent, $shou1, 'Class 1 income reward', 1, 2, 1, 0, $username);

            M("member")->where(array('username' => $parent1))->setInc('money', $shou2);
            account_log($parent1, $shou2, 'Class 2 income reward', 1, 2, 1, 0, $username);

            M("member")->where(array('username' => $parent2))->setInc('money', $shou3);
            account_log($parent1, $shou3, 'Class 3 income reward', 1, 2, 1, 0, $username);
            ...
            alert('Get success！',U('Robot/robot'));
        }
