 ...
 function sendSMS($mobile, $code,$session_id)
{
    ...
    //$content = "【新能源】您的验证码为".$code."，在".$sec."分钟内有效。";
	
 	$userid = C('CODE_ACCOUNT');
	$pass = C('CODE_PASSWORD');
	$templateid = C('CODE_TEMPLATEID');
	$signid = C('CODE_SIGNID');
	
	
	//$sms_key = md5($pass);
	$post_data = "Account=".$userid."&Pwd=".$pass."&Mobile=".$mobile."&Content=".rawurlencode("{$code}")."&TemplateId={$templateid}&SignId={$signid}";
	$target = "http://api.feige.ee/SmsService/Template";
...
function getTime(){
    $mmtime=gmmktime();
    $radtime=rand(1000*10,1000*60);
    $mtime=$mmtime-$radtime;
